...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.velocity = 75  # 속도도 0인 상태로 변수 설정
    ...
    def PID(self, kP = 50/100, kD = 0/100):
        ...
        if line is None:                    # line 이 없으면
            ...
            if(self.vars.noLine>=15):
                if self.vars.traffic == True:
                    ...
                    self.vars.traffic_control = 0
                if self.vars.traffic_control ==1:
                    if(round(time.time(),2)<(starttime+2)):
                        return -15, self.vars.velocity
                    
            
            #if all(self.maxPullMem > -10) and all(self.maxPullMem < 10):
           # print('none')
            if self.error_current > -5 and all(self.maxPullMem != -self.maxPullAng) and self.vars.steer > -5:
