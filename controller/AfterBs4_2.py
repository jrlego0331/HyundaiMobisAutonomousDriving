...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.velocity = 70  # 속도도 0인 상태로 변수 설정
        
        #PID control var
        self.error_current = 0
        self.HW_error_calibration = 2
        self.error_min_threshold = 0
        self.error_max_threshold = 30   

        self.maxPullMem = np.zeros(40)
        self.maxPullAng = 75

    def PID(self, kP = 40/100, kD = 0/100):
...
'''
        if line is None:                    # line 이 없으면
           # print('none')
            if self.error_current > -10 and all(self.maxPullMem != -self.maxPullAng) and self.vars.steer > -10:
