...
class Planning(BasePlanning):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if self.vars.redCnt >= 10:    # 5는 임의로 넣어둠 / 직진 주행이 완성되면 그 속도에 맞춰 경험적으로 구하기 (2021.02.03)
...
'''
        if line is None:                    # line 이 없으면
            #if all(abs(self.maxPullMem) < 12):
             #   return -15, self.vars.velocity
           # print('none')
            if self.error_current > -5 and all(self.maxPullMem != -self.maxPullAng) and self.vars.steer > -0:
