...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.velocity = 60  # 속도도 0인 상태로 변수 설정
        
        #PID control var
        self.error_current = 0
        self.HW_error_calibration = 0
        self.error_min_threshold = 0
        self.error_max_threshold = 60

        self.maxPullMem = np.zeros(40)
        self.maxPullAng = 75
        self.run = False
        self.runcount = 0

    def PID(self, kP = 45/100, kD = 0/100):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if self.vars.redCnt >= 15:    # 5는 임의로 넣어둠 / 직진 주행이 완성되면 그 속도에 맞춰 경험적으로 구하기 (2021.02.03)
        ...
        if frontLidar < 200 and frontLidar > 10:
            return self.vars.steer ,0
...
'''
        ...
        if self.run == True and self.runcount < 30:
            self.runcount+=1
            return -15, self.vars.velocity
        else:
            self.runcount = 0
            self.run = False

        if line is None:                    # line 이 없으면
            if all(abs(self.maxPullMem) < 10):
                return -8, self.vars.velocity
           # print('none')
            if self.error_current > -5 and all(self.maxPullMem != -self.maxPullAng) and self.vars.steer > -10:
                self.maxPullMem = np.delete(self.maxPullMem, 0)
                #self.vars.steer = 85
                self.vars.velocity = 80
                #self.vars.steer = self.maxPullAng * 0.8
                self.maxPullMem = np.append(self.maxPullMem, self.maxPullAng)
                return self.maxPullAng, self.vars.velocity
            ...
            self.error_current = e * 0.5
           # print('R: ', self.error_current)
       ...
       # self.maxPullMem = np.zeros(20)
        ...
        self.maxPullMem = np.append(self.maxPullMem, self.vars.steer)
        self.maxPullMem = np.delete(self.maxPullMem, 0)
